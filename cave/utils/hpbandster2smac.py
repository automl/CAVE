...
class HpBandSter2SMAC(object):
    ...
    def convert(self, folder):
        ...
        cs = self.load_configspace(folder)
    ...
    def load_configspace(self, folder):
        ...
        Hope this is fixed now with Pullrequest PRXXX"""
        ...
        if os.path.exists(cs_fn_json):
            ...
            self.logger.debug("Detected and loaded \"%s\"", cs_fn_json)
    ...
    def _get_config(self, config_id, id2config, cs):
        # fix datatypes of configuration according to them stored in the configuration space.
        id2config[config_id]['config'] = self._fix_types(cs, id2config[config_id]['config'])
    ...
    def _fix_types(self, cs, cfg):
        """
        Iterate over the configuration, which is read in for example from the BOHB run results and fixes the datatype
        for each hyperparameter according to the configuration space definition,
        which is read in from the configspace definition.

        This fix is necessary since the restored configuration space suffers from type uncertainties. This means, some
        hyperaparameter are stored as strings but where initially bool, int or floats.

        Args:
            cs: ConfigSpace.ConfigurationSpace, which is restored from the stored configspace.json or configspace.pcs
            cfg: a ConfigSpace.configuration, read in from the run results

        Returns:
            ConfigSpace.configuration, with same datatypes as in the ConfigurationSpace definition.
        """
        for key in cfg:
            if issubclass(type(cs.get_hyperparameter(key)), IntegerHyperparameter):
                type_hp = int

            elif issubclass(type(cs.get_hyperparameter(key)), FloatHyperparameter):
                type_hp = float

            elif type(cs.get_hyperparameter(key)) is CategoricalHyperparameter:
                type_hp = type(cs.get_hyperparameter(key).choices[0])

            else:
                raise ValueError('Hyperparametertype not understood. Must be one of IntegerHyperparameter,'
                                 ' FloatHyperparameter, CategoricalHyperparameter. But found {}'.format(
                                  type(cs.get_hyperparameter(key))))

            cfg[key] = type_hp(cfg[key])
        return cfg
