...
def get_checkbox(glyph_renderers, labels):
    ...
    for group in glyph_renderers:
        ...
        start += len(group)
    #self.logger.debug("Checkbox log: #renderers %d, #aliases %d, #all lines %d", len(glyph_renderers), len(aliases), num_total_lines)
    ...
    args_checkbox = {name: glyph for name, glyph in zip(aliases_flattened, glyph_renderers_flattened)}
    ...
    code_all = "var labels = " + handle_list_as_string + "; checkbox.active = labels;" + code.replace('cb_obj', 'checkbox')
    code_none = "var labels = []; checkbox.active = labels;" + code.replace('cb_obj', 'checkbox')
    select_all = Button(label="All", callback=CustomJS(args=dict({'checkbox':checkbox}, **args_checkbox), code=code_all))
    select_none = Button(label="None", callback=CustomJS(args=dict({'checkbox':checkbox}, **args_checkbox), code=code_none))
